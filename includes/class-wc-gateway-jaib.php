...
function jaib_initiate_connection() {
    ...
    class WC_Gateway_Jaib extends WC_Payment_Gateway {
        ...
        public function log_successful_payment($data) {
            ...
            if (function_exists('update_order_status_after_payment')) {
                update_order_status_after_payment($data['order_id']);
            }
        }
        public function payment_scripts() {
            ...
            wp_localize_script('jaib-scripts', 'jaib_script_vars', array(
                ...
                'userName' => esc_js($this->get_userName()),
                'password' => esc_js($this->get_password()),
                'agentCode' => esc_js($this->get_agentCode()),
                'loginUrl' => esc_js($this->get_loginUrl()),
                'order_id' => get_the_ID()
            ));
        }
